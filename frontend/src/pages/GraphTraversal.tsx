...
const GraphTraversal: React.FC = () => {
    ...
    const handleAddConnectedNode = (x: number, y: number, fromId: string) => {
        ...
        setNodes(prev => [...prev, { id, x, y }]);
        setEdges(prev => {
            // Avoid duplicate entries if something weird happens, but generally safe
    ...
    const handleRun = async () => {
        ...
        try {
            ...
            // Will fill up as we step? 
            // Actually result.order is the final BFS/DFS order. 
            // But we want to show it building up. 
            // The 'visited' list in result.steps[i] effectively shows the order so far.
            // Let's use that.
    ...
    // Helper to extract traversal order from visited history used for the final summary panel
    // We can just use the backend "order" if we want, but let's derive it to be synced with step
    // Actually currentVisited IS the traversal order so far in our backend logic? 
    // Yes, BFS/DFS Logic: 'visited' is a list of nodes in order of visitation.
