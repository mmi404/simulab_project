...
const RAGVisualization: React.FC<RAGVisualizationProps> = ({
    ...
    const getClickedNode = (clientX: number, clientY: number) => {
        ...
        for (const [node, pos] of nodePositionsRef.current.entries()) {
            // Use bounding box for easier clicking (especially for square nodes)
    ...
    const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        if (clickedNode) {
            ...
            // Initialize drag position to center of start node for smoothness
    ...
    const handleMouseUp = (e: React.MouseEvent<HTMLCanvasElement>) => {
        if (dragStartNode) {
            ...
            if (droppedNode && droppedNode !== dragStartNode) {
                ...
                // Valid connections: Process <-> Resource
    ...
    useEffect(() => {
        ...
        const render = () => {
            // Resize handling
            // Resize handling - using getBoundingClientRect for 1:1 pixel mapping
            const rect = canvas.getBoundingClientRect();
            ...
            // Processes on the Left (20% width)
            ...
            // Resources on the Right (80% width)
            ...
            if (dragStartNode && dragCurrentPos && nodeCoordinates[dragStartNode]) {
                ...
                drawArrow(ctx, start.x, start.y, dragCurrentPos.x, dragCurrentPos.y, '#ffffff', 2, 0); // 0 radius for mouse pointer
            }
        ...
        const drawNodes = (ctx: CanvasRenderingContext2D, nodeCoordinates: Record<string, { x: number, y: number }>, radius: number) => {
            ...
            allNodes.forEach(node => {
                ...
                if (node === dragStartNode) {
                    fillStyle = '#ffff00'; // Yellow for selection source
