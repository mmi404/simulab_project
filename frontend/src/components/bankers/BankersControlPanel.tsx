...
const BankersControlPanel: React.FC<BankersControlPanelProps> = ({ config, onConfigChange, onRun, onReset, speed, onSpeedChange }) => {
    ...
    // Updates available based on total - sum(allocated)
    ...
    // Helper to trigger config update with auto-calculation
    ...
    const removeResource = (index: number) => {
        if (config.resources.length <= 1) return; // Prevent deleting last resource
    ...
    const removeProcess = (index: number) => {
        if (config.processes.length <= 1) return; // Prevent deleting last process

        const newProcesses = config.processes.filter((_, i) => i !== index);

        // Re-index process IDs? Optional but cleaner
            ...
            <div className="space-y-2">
                ...
                <div className="flex items-center gap-4 bg-gray-50 p-2 rounded-lg border">
                    ...
                    <input
                        ...
                        max="3000"
                        step="250"
                        // Reverse value so slider right = faster? Or slider right = slower (higher ms)?
                        // Usually right = faster. So slider value should be high = small delay.
                        // Let's stick to "Step Delay" for clarity if we show ms, or abstract it.
                        // Let's do: Slider Left = Slow (3000ms), Right = Fast (500ms).
                        value={3500 - speed}
